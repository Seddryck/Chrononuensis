...
$projects = @(
  "src/Chrononuensis.Core/Chrononuensis.Core.csproj",
  "src/Chrononuensis.Cli/Chrononuensis.Cli.csproj"
)
...
foreach ($project in $projects) {
    foreach ($framework in $frameworks) {
        foreach ($runtime in $runtimes) {
            ...
            if ($project -like "*Chrononuensis.Cli*") {
                ...
                7z a ./.publish/Chrononuensis-$env:GitVersion_SemVer-$framework-$runtime.zip ./.publish/$framework/$runtime/*.*
            }
        }
    }
